...
| F2 | AL has no conditional expression (no ternary). | Mutants are lifted to statement level. |
...
- File: first object keyword is not `codeunit` → `not-a-codeunit`; path contains `Obsolete Objects` → `obsolete-folder`; object has `Subtype = Test` → `test-codeunit`; file name ends with `.Test.al` → `test-codeunit`.
...
| `Compile-MutApp -Env -Path [-Ruleset]` | `@{ Success; Diagnostics; AppFile; DurationSec }` | `compile <Path> --json [--ruleset <Ruleset>] --no-raw-output`; `Diagnostics` = `[{Severity; Code; File; Line; Column; Message}]`; `AppFile` = newest `*.app` in `<Path>` |
