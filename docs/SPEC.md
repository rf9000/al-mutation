...
2. `Ensure-MutEnvironment` — `Get-MutEnvironment` or `New-MutEnvironment` (`-SkipEnvironment` requires an existing one), else `Start-MutEnvironment` unconditionally (F3, finding I6 — not only when not already `Running`, since a `Running` status is not proof of serving). Both `New-MutEnvironment` and this call pass `-RequireProbe $false` (F3b IMPORTANT 2): the settle-and-probe's target is the AUT's own test codeunit, which does not exist until step 3 installs it, so an unconfirmed probe here is a warning, not fatal. `Sync-MutAutCopy`.
...
9. `Export-Results` — GET all `mutantResults` for `RunNo`, merge with `mutants.json` → `results/<RunNo>.json` (§7.3) and `results/<RunNo>-summary.md` (§7.5). `Remove-MutEnvironment` unless `keepEnvironment`. **On the step-8 cap-abort path (F3b): still writes `results/<RunNo>.json`/summary (every mutant without a row renders as `Pending`, §7.3) and Write-Warnings how many of how many mutants finished, but does not write `export.done` and does not call `Remove-MutEnvironment` regardless of `keepEnvironment` — then the run still throws (never reports success) after this partial export lands on disk.**
