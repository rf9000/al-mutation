...
8. `Invoke-MutantLoop` — §6.5.6. Re-invoking `Invoke-MutationRun.ps1` with the same `-RunNo` after this step has started (e.g. after a crash) continues where the loop stopped, rather than re-running every mutant from scratch (§6.5.6).
...
3. Run `Invoke-MutTests` on a background runspace under the budget (§6.5.6 implementation note: a runspace, not `Start-Job` — see the FIX comment on `Invoke-MutTestsWithBudget`). On timeout: `Reset-MutEnvironment`, record `Timeout`, PATCH `activeMutantId = 0`, continue.
...
6. Each mutant's entire body (steps 1–5) is wrapped so an unexpected error of any kind — not just a timeout or a `Failed`/`Passed` outcome — records `Status = 'Error'` with the exception message and moves on to the next mutant, never aborting the run. The loop reports how many mutants ended in `Error` when it finishes.
